...
{% hint style="info" %}
**v3** — any listener below can be selectively disabled for optimization via [`Config.DisabledListeners`](../configuration.md#disabled-listeners). If you don't react to an event from another resource, disable it to reduce network traffic.
{% endhint %}
...
Triggered when a skill's degradation XP hits 0 and the skill effect is disabled. See the [Skill Degradation](../degradation.md) page for the full lifecycle.
