...
{% hint style="info" %}
**v3** — server-side scripts also have access to global [`Helpers`](helpers.md) (jobs, items, money, identifiers, admin checks). These are NOT exports — they're loaded globally from `configs/s.helpers.lua` for use inside unlock handlers, visibility handlers, and the premium currency handler.
{% endhint %}
