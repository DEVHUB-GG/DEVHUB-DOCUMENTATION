...
{% hint style="info" %}
**v3** — pre-made skill effects can now be subject to the optional [Skill Degradation](../degradation.md) system if you enable it. Effects below describe the **active** state; when a skill is depleted by degradation, its effect is disabled until reactivation.
{% endhint %}
...
v3 expands the built-in activities that grant XP. All are configured via [`Config.EarnXp`](../configuration.md#xp-earning-activities):
