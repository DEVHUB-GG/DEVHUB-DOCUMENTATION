...
You can also use the new [Helpers](development/helpers.md) (v3) to simplify item/job/money checks.
...
A: A global secondary currency you can require (or accept) to unlock skills — useful for donor/VIP currencies. Configured via `Config.PremiumCurrency` with a server-side `handler` that checks and deducts the currency. Skills opt-in by specifying a `premiumCurrency` amount via the generator. See the [Premium Currency section](configuration.md#premium-currency) in the configuration.
...
{% content-ref url="migration-v2-to-v3.md" %}
