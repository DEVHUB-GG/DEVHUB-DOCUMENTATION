---
description: >-
  In-game admin dashboard for managing player XP, skill points, and unlocked
  skills. New in v3.
---
...
Admin status is determined by your devhub_lib framework adapter — typically the same logic that decides if `/admin` and similar commands work in your other resources. There is no separate ACE / group config for the skill tree itself; if `Core.IsPlayerAdmin(source)` returns `true`, the player can use the panel.
...
This is why v3 adds the `name` column — without it, the panel can't search for offline players by name.
